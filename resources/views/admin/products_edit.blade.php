@extends("admin_base")
@section("title") Edit Product - {{ $product->title }} @endsection
...
@section("content")
<div class="mb-2">
<h5 class="text-dark border-bottom"><a href="{{ route('admin_products_list') }}" class="text-muted">Products List</a>/Edit  Product - {{ $product->slug }}</h5>
</div>
<form action="{{ route('admin_products_edit_handler') }}" method="POST" enctype="multipart/form-data">
@csrf
<div class="row">
...
<div class="mb-3">
    ...
    <small class="text-muted" id="product_slug_preview">product url is: <span class="text-info">{{ $product->slug }}</span></small>
    <input type="hidden" name="slug" id="product_slug_field" value="{{ $product->slug }}">
</div>
<div class="mb-3">
    <label for="editor_description" class="text-dark mb-1">Description</label>
    <textarea name="description" id="editor_description">{{ $product->description }}</textarea>
</div>
<div class="mb-3">
    <label for="editor_details" class="text-dark mb-1">Details</label>
    <textarea name="details" id="editor_details">{{ $product->details }}</textarea>
</div>
<div class="mb-3">
    <label for="editor_materials" class="text-dark mb-1">Materials</label>
    <textarea name="materials" id="editor_materials">{{ $product->materials }}</textarea>
</div>
<div class="mb-3">
    <label for="editor_measurement" class="text-dark mb-1">Measurement</label>
    <textarea name="measurement" id="editor_measurement">{{ $product->measurement }}</textarea>
</div>
<div>
    <label for="editor_care" class="text-dark mb-1">Care</label>
    <textarea name="care" id="editor_care">{{ $product->care }}</textarea>
</div>
</div>
</div>
<div class="col-md-6">
...
<select name="sub_category" id="sub_category" class="custom-select">
    ...
    <option value="{{ $scat->id }}" data-category="{{ $scat->category->title }}" {{ $scat->id === $product->sub_category_id ? "selected" : "" }}>{{ $scat->title }}</option>
    @endforeach
</select>
<small id="category_name">parent category: <span class="text-info">{{ $product->subCategory->category->title }}/{{ $product->subCategory->title }}</span></small>
</div>
<div class="col-md-6 mb-3 mb-md-0">
...
<div class="col-md-6">
    ...
    <div class="input-group">
        ...
        <input type="number" name="regular_price" id="regular_price" class="form-control" value="{{ $product->regular_price }}" placeholder="optional">
    </div>
</div>
</div>
...
<div class="col-12">
<hr>
<ul class="list-group" id="variant_list_group"></ul>
</div>
</div>
</div>
...
$(document).ready(function() {
    ...
    function updateVariantList() {
        ...
        variants.forEach((variant, index) => {
            let thumbnail = variant.thumbnail;
            if(variant.thumbnail.startsWith('data:')){
                thumbnail = variant.thumbnail;
            }else{
                thumbnail = '/storage/'+variant.thumbnail;
            }
            let listItem = $(`
                <li class="list-group-item p-2">
                    <div class="row">
                        <div class="col-12 col-md-6 d-flex">
                            <img src="${thumbnail}" class="img-fluid mr-2" width="80px">
